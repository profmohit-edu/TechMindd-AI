...
import { BookOpenCheck, Boxes, BrainCircuit, BriefcaseBusiness, Database, Gauge, Menu, Moon, Plug, Settings, Sparkles, Sun, X } from 'lucide-react'

const links=[['/',Gauge,'Dashboard'],['/learn',BookOpenCheck,'Learning Assistant'],['/generate',Sparkles,'Generate'],['/jobs',BriefcaseBusiness,'Jobs'],['/packages',Boxes,'Packages'],['/knowledge',Database,'Knowledge Base'],['/plugins',Plug,'Plugins'],['/providers',BrainCircuit,'Providers'],['/settings',Settings,'Settings']] as const
export default function Layout(){
 ...
 return <div className="min-h-screen text-slate-900 dark:text-slate-100">
  ...
  <div className="lg:pl-72"><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/80 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80 sm:px-8"><button className="lg:hidden" onClick={()=>setOpen(true)}><Menu/></button><div className="hidden text-sm text-slate-500 sm:block">AI-enabled technical learning and content platform</div><button className="rounded-xl border border-slate-200 p-2 dark:border-slate-700" onClick={()=>setDark(!dark)}>{dark?<Sun size={18}/>:<Moon size={18}/>}</button></header><main className="p-4 sm:p-8"><Outlet/></main></div>
 </div>
}
